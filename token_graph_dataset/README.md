...
從已保存的 Ethereum Uniswap V2 `Sync` 紀錄重建小型 DELTA 資料集。使用真實鏈上觀測的準備金，不隨機生成市場權重，也不修改資料來植入負環。這個版本是**紀錄重建器**，不含新的 RPC 蒐集器、不會連網或送交易。

只依賴 Python 標準函式庫與 repository 內的 DELTA；不匯入封存研究模組。原始市場資料與產出不隨 Git repository 提供。首次使用前先準備符合下列契約的紀錄資料夾，不能把作者 UNI 檔案直接當成這裡的來源。
...
第一版範圍：Ethereum mainnet、Uniswap V2、固定 997/1000 費率、1–8 個池、最多 10 個代幣、1–1,000 區塊、最多 10,000 筆更新 Sync。小圖枚舉驗證另限最多 1,000 個交易批次、`k=2–5` 且代幣數至少 k。預設 `k=3`。這是功能與正確性檢查，不是大規模效能測試。
...
驗證先從 `source.json` 重新建圖，與磁碟 case 精確比較，再執行 DELTA。獨立 oracle 以完整排列枚舉恰好 k 條邊的簡單有向環，分別求固定著色限制與全圖最小值；權重及見證採 `abs=1e-10, rel=1e-10` 容差。再用原始整數準備金的精確分數乘積判定所選見證環的費後匯率是否大於 1，避免僅憑浮點近零負號解讀訊號。

顏色固定為 `node_id % k`；三個代幣、k=3 時涵蓋全圖三跳環，較大圖不保證全圖覆蓋。驗證分開回報 `restricted_matches`、`global_matches` 與負環狀態數，沒有負環是有效結果。時間欄位僅作執行紀錄，重播一致性比較應排除 `update_ms`／`query_ms`。

負環不等於可成交淨利；未納入滑價、交易量、gas、MEV 競爭與執行。這個三池 pilot 不支持 k=4／5、市場全面泛化或 UNI 等級規模的效能結論。

## 測試

```sh
python -m delta_terminal build
python -m unittest token_graph_dataset.tests.test_dataset delta_terminal.tests.test_terminal -v
```
